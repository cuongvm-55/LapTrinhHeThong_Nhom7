...
void initProduct() {
	products = (struct product *) malloc(sizeof(struct product) * productSize);
...
void on_next_step_clicked(GtkWidget *button, gpointer data) {
	if (current_step == 1) {
		...
		// Send customer data to server, include: customer id, message
		...
		for (int i = 0; i < productSize; i++) {
			printf("Sending product %d\n", i);
			struct product current_product = products[i];

			json_t *data_2 = json_object();

			// convert int value to string
			char * str_product_id = (char *) malloc(10*sizeof(char));
			char * str_quantity = (char *) malloc(10*sizeof(char));
			sprintf(str_product_id, "%d", current_product.productId);
			sprintf(str_quantity, "%d", current_product.current_quantity);

			// append value to json data
			json_object_set(data_2, "oid", json_string("1"));
			json_object_set(data_2, "pid", json_string(str_product_id));
			json_object_set(data_2, "quantity", json_string(str_quantity));

			// send data
			char* r_2_string = get_data("porderdetail", data_2);
...
char *printProduct(struct product product) {
	...
	sprintf(content, "%s\t%s\nSo luong: %d\tDon gia: %d VND\tGiam gia: %d percent",
		product.productName, product.brand, product.current_quantity, product.price, product.saleOff);
...
void fillAllProducts() {
	char *content = (char *) malloc(800*sizeof(char));
	sprintf(content, "");
	...
	gtk_label_set_text(GTK_LABEL(label_products), content);
}
